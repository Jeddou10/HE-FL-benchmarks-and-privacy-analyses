...
- Measures a broad set of metrics:
  ...
  * HE setup time (context creation & keygen)
  * Per-round encryption, serialization size (communication), aggregation (on encrypted data), decryption, and apply time
  * Plaintext FedAvg aggregation time (baseline)
  * Test accuracy / loss per round
  * Peak RAM (optional) and CPU usage (best-effort)
- Supports IID or Dirichlet non-IID partitioning.
...
def deserialize_ciphertexts(public_ctx, blobs: list):
    return [ts.ckks_vector_from(public_ctx, b) for b in blobs]
...
def run_benchmark(args):
    ...
    with open(log_csv, 'w') as f:
        f.write(
            "round,clients,train_ms,he_setup_ms,he_encrypt_ms,he_bytes,total_bytes,he_aggregate_ms,he_decrypt_ms,apply_ms,pt_aggregate_ms,test_acc,test_loss,peak_rss_mb\n"
        )
    ...
    for r in range(1, args.rounds + 1):
        ...
        # Apply FedAvg weights on the client side BEFORE encryption.
        ...
        with open(log_csv, 'a') as f:
            f.write(
                f"{row['round']},{row['clients']},{row['train_ms']},{row['he_setup_ms']},{row['he_encrypt_ms']},{row['he_bytes']},{row['total_bytes']},{row['he_aggregate_ms']},{row['he_decrypt_ms']},{row['apply_ms']},{row['pt_aggregate_ms']},{row['test_acc']:.4f},{row['test_loss']:.4f},{row['peak_rss_mb']:.2f}\n"
            )

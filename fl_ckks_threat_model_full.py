"""
FL + CKKS (TenSEAL) MNIST — Comprehensive HE Threat Model & Attacks
----------------------------------------------------------------------------
This script adapts the comprehensive threat-model harness (gradient inversion,
membership inference, probing, timing leakage, collusion) to the HE
benchmark.
"""
...
def run_full_threat_model(args):
    ...
    for r in range(1, args.rounds + 1):
        ...
        if args.attack_inversion and r == args.attack_round:
            ...
            # ALSO save reconstructed image: inversion.png
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Comprehensive HE threat-model harness (MNIST + CKKS)')
    ...
    # Local training batch size for each client.

    parser.add_argument('--lr', type=float, default=0.1)
    # Learning rate for clients' local optimizer (SGD).

    parser.add_argument('--attack-round', type=int, default=5)
    # Round index at which attacks are executed (e.g., run privacy tests after this round).

    parser.add_argument('--attack-all', action='store_true')
    # Convenience flag: if set, enables all attack types.

    parser.add_argument('--attack_inversion', action='store_true')
    # Enable optimization-based gradient inversion attacks.

    parser.add_argument('--attack_membership', action='store_true')
    # Enable loss-threshold membership inference attacks.
    ...
    # Enable timing and serialized-size leakage analysis.

    parser.add_argument('--attack_steps', type=int, default=800)
    # Number of optimization steps used by inversion-style attacks.

    parser.add_argument('--attack_lr', type=float, default=0.05)
    # Learning rate for the inversion optimizer.

    parser.add_argument('--attack_restarts', type=int, default=2)
    # Number of random restarts for the inversion optimizer to avoid local minima.

    parser.add_argument('--attack_slice_idx', type=int, default=0)
    # Index of the model parameter slice/block to attack when slicing is used.

    parser.add_argument('--attack_client', type=int, default=None)
    # Specific client id to target for attacks (None => use default/selected client).

    parser.add_argument('--membership_threshold', type=float, default=0.5)
    # Threshold used by simple membership inference (loss-threshold) to decide membership.

    parser.add_argument('--probe_scale', type=float, default=1e-2)
    # Magnitude scaling factor for probing/marker injection.

    parser.add_argument('--n_aggregators', type=int, default=3)
    # Number of aggregator replicas simulated (used in certain collusion scenarios).
    ...
    # Number of epochs to train each shadow model for shadow-based membership inference.
    ...
    # Percentile used to derive a membership decision threshold from shadow-model losses.

    parser.add_argument('--poly-mod-degree', type=int, default=16_384)
    # CKKS polynomial modulus degree (affects slot capacity & security level).
    ...
    # RNG seed for reproducibility (partitions, training, DP randomness, attacks, etc.).

    args, unknown = parser.parse_known_args()
    # Parse known CLI args and collect any unknown extras into `unknown` (useful in wrapped contexts).

    # enable all attacks if user asked (force-on here for demo)
    args.attack_inversion = True
    # Force inversion attacks to be enabled (overrides CLI); used here to ensure demos run with inversion.

    args.attack_membership = True
    # Force membership inference to be enabled.

    args.attack_membership_shadow = True
    # Force shadow-model membership inference to be enabled.

    args.attack_probing = True
    # Force probing/marker attacks to be enabled.

    args.attack_collusion = True
    # Force collusion simulations to be enabled.

    args.attack_timing = True
    # Force timing/size-leakage analysis to be enabled.
